-- Copyright 1986-2018 Xilinx, Inc. All Rights Reserved.
-- --------------------------------------------------------------------------------
-- Tool Version: Vivado v.2018.3 (win64) Build 2405991 Thu Dec  6 23:38:27 MST 2018
-- Date        : Wed Mar 27 06:28:50 2019
-- Host        : Oz-Bejerano-Laptop running 64-bit major release  (build 9200)
-- Command     : write_vhdl -force -mode funcsim {C:/Users/Oz
--               Bejerano/PycharmProjects/RISC-V-Processor/VHDL/Proj/risc_v_test
--               project.srcs/sources_1/bd/RV32I_pipelined/ip/RV32I_pipelined_hazard_logic_0_0/RV32I_pipelined_hazard_logic_0_0_sim_netlist.vhdl}
-- Design      : RV32I_pipelined_hazard_logic_0_0
-- Purpose     : This VHDL netlist is a functional simulation representation of the design and should not be modified or
--               synthesized. This netlist cannot be used for SDF annotated simulation.
-- Device      : xc7z010clg400-1
-- --------------------------------------------------------------------------------
library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
library UNISIM;
use UNISIM.VCOMPONENTS.ALL;
entity RV32I_pipelined_hazard_logic_0_0 is
  port (
    instr_f_d : in STD_LOGIC_VECTOR ( 31 downto 0 );
    instr_d_e : in STD_LOGIC_VECTOR ( 31 downto 0 );
    instr_e_m : in STD_LOGIC_VECTOR ( 31 downto 0 );
    instr_m_w : in STD_LOGIC_VECTOR ( 31 downto 0 );
    hazard_stage : out STD_LOGIC_VECTOR ( 2 downto 0 );
    new_hazard : out STD_LOGIC
  );
  attribute NotValidForBitStream : boolean;
  attribute NotValidForBitStream of RV32I_pipelined_hazard_logic_0_0 : entity is true;
  attribute CHECK_LICENSE_TYPE : string;
  attribute CHECK_LICENSE_TYPE of RV32I_pipelined_hazard_logic_0_0 : entity is "RV32I_pipelined_hazard_logic_0_0,hazard_logic,{}";
  attribute downgradeipidentifiedwarnings : string;
  attribute downgradeipidentifiedwarnings of RV32I_pipelined_hazard_logic_0_0 : entity is "yes";
  attribute ip_definition_source : string;
  attribute ip_definition_source of RV32I_pipelined_hazard_logic_0_0 : entity is "module_ref";
  attribute x_core_info : string;
  attribute x_core_info of RV32I_pipelined_hazard_logic_0_0 : entity is "hazard_logic,Vivado 2018.3";
end RV32I_pipelined_hazard_logic_0_0;

architecture STRUCTURE of RV32I_pipelined_hazard_logic_0_0 is
  signal \^hazard_stage\ : STD_LOGIC_VECTOR ( 2 downto 0 );
  signal hazard_stage10_in : STD_LOGIC;
  signal hazard_stage12_in : STD_LOGIC;
  signal hazard_stage15_in : STD_LOGIC;
  signal \hazard_stage[0]_INST_0_i_2_n_0\ : STD_LOGIC;
  signal \hazard_stage[0]_INST_0_i_3_n_0\ : STD_LOGIC;
  signal \hazard_stage[0]_INST_0_i_4_n_0\ : STD_LOGIC;
  signal \hazard_stage[0]_INST_0_i_5_n_0\ : STD_LOGIC;
  signal \hazard_stage[1]_INST_0_i_2_n_0\ : STD_LOGIC;
  signal \hazard_stage[1]_INST_0_i_3_n_0\ : STD_LOGIC;
  signal \hazard_stage[1]_INST_0_i_4_n_0\ : STD_LOGIC;
  signal \hazard_stage[1]_INST_0_i_5_n_0\ : STD_LOGIC;
  signal \hazard_stage[2]_INST_0_i_2_n_0\ : STD_LOGIC;
  signal \hazard_stage[2]_INST_0_i_3_n_0\ : STD_LOGIC;
  signal \hazard_stage[2]_INST_0_i_4_n_0\ : STD_LOGIC;
  signal \hazard_stage[2]_INST_0_i_5_n_0\ : STD_LOGIC;
begin
  hazard_stage(2 downto 0) <= \^hazard_stage\(2 downto 0);
\hazard_stage[0]_INST_0\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"AA808080"
    )
        port map (
      I0 => hazard_stage10_in,
      I1 => \hazard_stage[0]_INST_0_i_2_n_0\,
      I2 => \hazard_stage[0]_INST_0_i_3_n_0\,
      I3 => \hazard_stage[0]_INST_0_i_4_n_0\,
      I4 => \hazard_stage[0]_INST_0_i_5_n_0\,
      O => \^hazard_stage\(0)
    );
\hazard_stage[0]_INST_0_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFE"
    )
        port map (
      I0 => instr_m_w(10),
      I1 => instr_m_w(11),
      I2 => instr_m_w(9),
      I3 => instr_m_w(7),
      I4 => instr_m_w(8),
      O => hazard_stage10_in
    );
\hazard_stage[0]_INST_0_i_2\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_m_w(11),
      I1 => instr_f_d(19),
      I2 => instr_m_w(10),
      I3 => instr_f_d(18),
      I4 => instr_f_d(17),
      I5 => instr_m_w(9),
      O => \hazard_stage[0]_INST_0_i_2_n_0\
    );
\hazard_stage[0]_INST_0_i_3\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(15),
      I1 => instr_m_w(7),
      I2 => instr_f_d(16),
      I3 => instr_m_w(8),
      O => \hazard_stage[0]_INST_0_i_3_n_0\
    );
\hazard_stage[0]_INST_0_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_m_w(11),
      I1 => instr_f_d(24),
      I2 => instr_m_w(10),
      I3 => instr_f_d(23),
      I4 => instr_f_d(22),
      I5 => instr_m_w(9),
      O => \hazard_stage[0]_INST_0_i_4_n_0\
    );
\hazard_stage[0]_INST_0_i_5\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(20),
      I1 => instr_m_w(7),
      I2 => instr_f_d(21),
      I3 => instr_m_w(8),
      O => \hazard_stage[0]_INST_0_i_5_n_0\
    );
\hazard_stage[1]_INST_0\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"AA808080"
    )
        port map (
      I0 => hazard_stage12_in,
      I1 => \hazard_stage[1]_INST_0_i_2_n_0\,
      I2 => \hazard_stage[1]_INST_0_i_3_n_0\,
      I3 => \hazard_stage[1]_INST_0_i_4_n_0\,
      I4 => \hazard_stage[1]_INST_0_i_5_n_0\,
      O => \^hazard_stage\(1)
    );
\hazard_stage[1]_INST_0_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFE"
    )
        port map (
      I0 => instr_e_m(10),
      I1 => instr_e_m(11),
      I2 => instr_e_m(9),
      I3 => instr_e_m(7),
      I4 => instr_e_m(8),
      O => hazard_stage12_in
    );
\hazard_stage[1]_INST_0_i_2\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_e_m(11),
      I1 => instr_f_d(19),
      I2 => instr_e_m(10),
      I3 => instr_f_d(18),
      I4 => instr_f_d(17),
      I5 => instr_e_m(9),
      O => \hazard_stage[1]_INST_0_i_2_n_0\
    );
\hazard_stage[1]_INST_0_i_3\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(15),
      I1 => instr_e_m(7),
      I2 => instr_f_d(16),
      I3 => instr_e_m(8),
      O => \hazard_stage[1]_INST_0_i_3_n_0\
    );
\hazard_stage[1]_INST_0_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_e_m(11),
      I1 => instr_f_d(24),
      I2 => instr_e_m(10),
      I3 => instr_f_d(23),
      I4 => instr_f_d(22),
      I5 => instr_e_m(9),
      O => \hazard_stage[1]_INST_0_i_4_n_0\
    );
\hazard_stage[1]_INST_0_i_5\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(20),
      I1 => instr_e_m(7),
      I2 => instr_f_d(21),
      I3 => instr_e_m(8),
      O => \hazard_stage[1]_INST_0_i_5_n_0\
    );
\hazard_stage[2]_INST_0\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"AA808080"
    )
        port map (
      I0 => hazard_stage15_in,
      I1 => \hazard_stage[2]_INST_0_i_2_n_0\,
      I2 => \hazard_stage[2]_INST_0_i_3_n_0\,
      I3 => \hazard_stage[2]_INST_0_i_4_n_0\,
      I4 => \hazard_stage[2]_INST_0_i_5_n_0\,
      O => \^hazard_stage\(2)
    );
\hazard_stage[2]_INST_0_i_1\: unisim.vcomponents.LUT5
    generic map(
      INIT => X"FFFFFFFE"
    )
        port map (
      I0 => instr_d_e(10),
      I1 => instr_d_e(11),
      I2 => instr_d_e(9),
      I3 => instr_d_e(7),
      I4 => instr_d_e(8),
      O => hazard_stage15_in
    );
\hazard_stage[2]_INST_0_i_2\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_d_e(11),
      I1 => instr_f_d(19),
      I2 => instr_d_e(10),
      I3 => instr_f_d(18),
      I4 => instr_f_d(17),
      I5 => instr_d_e(9),
      O => \hazard_stage[2]_INST_0_i_2_n_0\
    );
\hazard_stage[2]_INST_0_i_3\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(15),
      I1 => instr_d_e(7),
      I2 => instr_f_d(16),
      I3 => instr_d_e(8),
      O => \hazard_stage[2]_INST_0_i_3_n_0\
    );
\hazard_stage[2]_INST_0_i_4\: unisim.vcomponents.LUT6
    generic map(
      INIT => X"9009000000009009"
    )
        port map (
      I0 => instr_d_e(11),
      I1 => instr_f_d(24),
      I2 => instr_d_e(10),
      I3 => instr_f_d(23),
      I4 => instr_d_e(9),
      I5 => instr_f_d(22),
      O => \hazard_stage[2]_INST_0_i_4_n_0\
    );
\hazard_stage[2]_INST_0_i_5\: unisim.vcomponents.LUT4
    generic map(
      INIT => X"9009"
    )
        port map (
      I0 => instr_f_d(20),
      I1 => instr_d_e(7),
      I2 => instr_f_d(21),
      I3 => instr_d_e(8),
      O => \hazard_stage[2]_INST_0_i_5_n_0\
    );
new_hazard_INST_0: unisim.vcomponents.LUT3
    generic map(
      INIT => X"FE"
    )
        port map (
      I0 => \^hazard_stage\(0),
      I1 => \^hazard_stage\(2),
      I2 => \^hazard_stage\(1),
      O => new_hazard
    );
end STRUCTURE;
